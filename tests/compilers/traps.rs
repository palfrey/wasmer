...
fn test_trap_return(config: crate::Config) -> Result<()> {
    ...
    let hello_func = Function::new(&mut ctx.as_context_mut(), &hello_type, |_ctx, _| {
...
fn test_trap_trace_cb(config: crate::Config) -> Result<()> {
    ...
    let fn_func = Function::new(&mut ctx.as_context_mut(), &fn_type, |_ctx, _| {
...
fn trap_start_function_import(config: crate::Config) -> Result<()> {
    ...
    let func = Function::new(&mut ctx.as_context_mut(), &sig, |_ctx, _| {
...
fn rust_panic_start_function(config: crate::Config) -> Result<()> {
    ...
    let func = Function::new_native(&mut ctx.as_context_mut(), |_ctx: ContextMut<_>| {
        panic!("this is another panic")
    });
    let err = panic::catch_unwind(AssertUnwindSafe(|| {
        drop(Instance::new(
...
fn mismatched_arguments(config: crate::Config) -> Result<()> {
    ...
    assert_eq!(
        func.call(&mut ctx.as_context_mut(), &[Value::F32(0.0)])
            .unwrap_err()
            .message(),
        "Parameters of type [F32] did not match signature [I32] -> []",
    );
    assert_eq!(
        func.call(&mut ctx.as_context_mut(), &[Value::I32(0), Value::I32(1)])
            .unwrap_err()
            .message(),
...
fn call_signature_mismatch(config: crate::Config) -> Result<()> {
    ...
    let module = Module::new(&store, &binary)?;
    let mut ctx = WasmerContext::new(&store, ());
    let err = Instance::new(&mut ctx.as_context_mut(), &module, &imports! {})
        .err()
...
fn present_after_module_drop(config: crate::Config) -> Result<()> {
    ...
    assert_trap(func.call(&mut ctx.as_context_mut(), &[]).unwrap_err());

...
fn native_function_works_for_wasm(config: crate::Config) -> anyhow::Result<()> {
    ...
    let import_object = imports! {
        "env" => {
            "multiply" => Function::new_native(&mut ctx.as_context_mut(), |_ctx: ContextMut<_>, a: i32, b: i32| a * b),
        },
    };
...

    {
        ...
        let result = f.call(&mut ctx.as_context_mut(), 4, 6)?;
        assert_eq!(result, 10);
    }

    {
        let f: &Function = instance.exports.get("double_then_add")?;
        let result = f.call(&mut ctx.as_context_mut(), &[Value::I32(4), Value::I32(6)])?;
        assert_eq!(result[0], Value::I32(20));
    }

    {
        let dyn_f: &Function = instance.exports.get("double_then_add")?;
        let f: TypedFunction<(i32, i32), i32> = dyn_f.native(&mut ctx.as_context_mut()).unwrap();
        let result = f.call(&mut ctx.as_context_mut(), 4, 6)?;
        assert_eq!(result, 20);
    }
...
fn non_native_functions_and_closures_with_no_env_work(config: crate::Config) -> anyhow::Result<()> {
    ...
    let import_object = imports! {
        "env" => {
            "multiply1" => Function::new(&mut ctx.as_context_mut(), &ty, move |_ctx, args| {
            ...
            "multiply2" => Function::new(&mut ctx.as_context_mut(), &ty, move |ctx, args| {
                if let (Value::I32(v1), Value::I32(v2)) = (&args[0], &args[1]) {
            ...
            "multiply3" => Function::new_native(&mut ctx.as_context_mut(), |_ctx: ContextMut<_>, arg1: i32, arg2: i32| -> i32
                                                {arg1 * arg2 }),
            "multiply4" => Function::new_native(&mut ctx.as_context_mut(), |ctx: ContextMut<i32>, arg1: i32, arg2: i32| -> i32
                                                         {arg1 * arg2 * ctx.data() }),
        },
    };
    ...
    let test: TypedFunction<(i32, i32, i32, i32, i32), i32> = instance
        .exports
...
fn native_function_works_for_wasm_function_manyparams(config: crate::Config) -> anyhow::Result<()> {
    ...
    let import_object = imports! {
        "env" => {
            "longf" => Function::new_native(&mut ctx.as_context_mut(), long_f),
        },
    };
...

    {
        ...
        let f: TypedFunction<(), i64> = dyn_f.native(&mut ctx.as_context_mut()).unwrap();
        let result = f.call(&mut ctx.as_context_mut())?;
...

    {
        ...
        let f: TypedFunction<(u32, u32, u32, u32, u32, u16, u64, u64, u16, u32), i64> =
            dyn_f.native(&mut ctx.as_context_mut()).unwrap();
        let result = f.call(&mut ctx.as_context_mut(), 1, 2, 3, 4, 5, 6, 7, 8, 9, 0)?;
        assert_eq!(result, 1234567890);
    }
...
fn native_function_works_for_wasm_function_manyparams_dynamic(
    ...
    let import_object = imports! {
        "env" => {
            "longf" => Function::new(&mut ctx.as_context_mut(), FunctionType::new(vec![ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I32, ValueType::I64 , ValueType::I64 ,ValueType::I32, ValueType::I32], vec![ValueType::I64]), long_f_dynamic),
        },
    };
...
fn static_host_function_without_env(config: crate::Config) -> anyhow::Result<()> {
    ...
    {
        let f = Function::new_native(&mut ctx.as_context_mut(), f);
        let f_native: TypedFunction<(i32, i64, f32, f64), (f64, f32, i64, i32)> =
            f.native(&mut ctx.as_context_mut()).unwrap();
        let result = f_native.call(&mut ctx.as_context_mut(), 1, 3, 5.0, 7.0)?;
        assert_eq!(result, (28.0, 15.0, 6, 1));
    }

    // Native static host function that returns a tuple.
    {
        let long_f = Function::new_native(&mut ctx.as_context_mut(), long_f);
        let long_f_native: TypedFunction<
            (u32, u32, u32, u32, u32, u16, u64, u64, u16, u32),
            (u32, u64, u32),
        > = long_f.native(&mut ctx.as_context_mut()).unwrap();
        let result = long_f_native.call(&mut ctx.as_context_mut(), 1, 2, 3, 4, 5, 6, 7, 8, 9, 0)?;
        assert_eq!(result, (654321, 87, 09));
    }

    // Native static host function that returns a result of a tuple.
    {
        let f = Function::new_native(&mut ctx.as_context_mut(), f_ok);
        let f_native: TypedFunction<(i32, i64, f32, f64), (f64, f32, i64, i32)> =
            f.native(&mut ctx.as_context_mut()).unwrap();
        let result = f_native.call(&mut ctx.as_context_mut(), 1, 3, 5.0, 7.0)?;
        assert_eq!(result, (28.0, 15.0, 6, 1));
